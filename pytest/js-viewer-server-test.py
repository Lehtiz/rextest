...
def posixJsServer(serverScript):
    s = "./server --headless --run " + serverScript + " 2>&1 | tee " + serverOutput
...
def posixJsViewer(viewerScript):
    v = "./viewer --headless --run " + viewerScript + " 2>&1 | tee " + viewerOutput
    viewercode = posixRun(v)
    with open(exitfileViewer, 'w') as e:
        e.write(viewercode)

def posixTxmlServer(serverScript):
    s = "./server --headless --file " + testfile + " --run " + serverScript + " 2>&1 | tee " + serverOutput
    servercode = posixRun(s)
    with open(exitfileServer, 'w') as e:
        e.write(str(servercode))

def posixTxmlViewer(viewerScript):
    v = "./viewer --headless --storage " + os.path.dirname(testfile) + "/ --run " + viewerScript + " 2>&1 | tee " + viewerOutput
    viewercode = posixRun(v)
    with open(exitfileViewer, 'w') as e:
